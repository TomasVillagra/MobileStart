// DetalleInsumo.js — marco de imagen visible siempre (con o sin imagen)
...
import { useIsFocused } from '@react-navigation/native'; // agregado
...
export default function DetalleInsumo({ route, navigation }) {
  ...
  // Recarga automática al volver de editar
  useEffect(() => {
    if (isFocused) {
      setLoading(true);
      fetchData();
    }
  }, [isFocused, insumoId]); // se vuelve a ejecutar al volver al detalle
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        {/* Marco de imagen SIEMPRE visible */}
        <View style={styles.imageFrame}>
          {insumo?.imageUrl ? (
            <Image source={{ uri: insumo.imageUrl }} style={styles.imageInside} />
          ) : (
        ...
        <View style={styles.headerRow}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            <View
              style={[
                styles.estadoPill,
                (insumo?.estado || '').toLowerCase() === 'activo' ? styles.estadoActivo : styles.estadoInactivo,
              ]}
            >
              <Text style={styles.estadoText}>
                {(insumo?.estado || '').toLowerCase() === 'activo' ? 'Activo' : 'Inactivo'}
              </Text>
            </View>
            {esCritico && (
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.value}>{insumo?.unidad}</Text>
        </View>
        <View style={styles.row}>
          <Text style={styles.label}>Stock actual:</Text>
          <Text style={styles.value}>{insumo?.stockActual}</Text>
        </View>
        <View style={styles.row}>
          <Text style={styles.label}>Punto de reposición:</Text>
          <Text style={styles.value}>{insumo?.puntoReposicion}</Text>
        </View>
        <View style={styles.row}>
          <Text style={styles.label}>Stock mínimo:</Text>
          <Text style={styles.value}>{insumo?.stockMin}</Text>
        </View>
        <View style={styles.row}>
          <Text style={styles.label}>Stock máximo:</Text>
          <Text style={styles.value}>{insumo?.stockMax}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  criticoPill: { paddingHorizontal: 10, 
    paddingVertical: 6, 
    backgroundColor: '#F87171', 
    borderRadius: 999 },
  criticoText: { color: '#000', 
    fontWeight: '800', 
    fontSize: 11 },
